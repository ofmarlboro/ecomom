...
class Count_m extends CI_Model
{
    ...
    function __construct()
    {
		...
		public function basic_total_count($date){

			//오늘
			$today_time = strtotime($date);

			//어제
			$yester = strtotime('-1 day',$today_time);

			//최근 1주일
			$this_week = strtotime('-7 day',$today_time);

			//이번달
			$this_month = strtotime('first day of this month',$today_time);

			//오늘
			$sql = "select sum(unique_counter) as sum1 from dh_counter_main where date = '$today_time'";
			$query = $this->db->query($sql);
			$row = $query->row();
			$count['today'] = "0";
			if($row) $count['today'] = $row->sum1;

			//어제
			$sql = "select sum(unique_counter) as sum1 from dh_counter_main where date = '$yester'";
			$query = $this->db->query($sql);
			$row = $query->row();
			$count['yester'] = "0";
			if($row) $count['yester'] = $row->sum1;

			//최근 1주일
			$sql = "select sum(unique_counter) as sum1 from dh_counter_main where date between '$this_week' and '$today_time'";
			$query = $this->db->query($sql);
			$row = $query->row();
			$count['this_week'] = "0";
			if($row) $count['this_week'] = $row->sum1;

			//이번달
			$sql = "select sum(unique_counter) as sum1 from dh_counter_main where date between '$this_month' and '$today_time'";
			$query = $this->db->query($sql);
			$row = $query->row();
			$count['this_month'] = "0";
			if($row) $count['this_month'] = $row->sum1;
