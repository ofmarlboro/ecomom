<?
	if(!$this->input->get('cate_no')){
		alert(cdir()."/dh/sidedish_free1/?cate_no=2-12");
	}
...
?>
	...
	<script type="text/javascript">
		...
		function add_tmp_cart(deliv_date,goods_idx,goods_price,goods_name){
			$.ajax({
				type:"GET"
				,url:"<?=cdir()?>/dh/dh_ajax/?ajax=1&mode=free_cart_tmp&deliv_date="+encodeURIComponent(deliv_date)+"&goods_idx="+goods_idx+"&price="+goods_price+"&goods_name="+encodeURIComponent(goods_name)
				,dataType:"json"
				,cache:false
				,error:function(xhr){ console.log(xhr.responseText); }
				,success:function(data){
	...
	<div id="container">
		...
		<div class="mid_cate_wrap">
			<div class="inner">
				<ul class="mid_cate">
					<li <?if($this->input->get('cate_no') == "2-12"){?>class="on"<?}?>><a href="<?=cdir()?>/dh/sidedish_free1/?cate_no=2-12"><span class="icon i7"></span>
							<p class="tit"><em>반찬</em></p>
						</a>
					</li>
					<li <?if($this->input->get('cate_no') == "2-13"){?>class="on"<?}?>><a href="<?=cdir()?>/dh/sidedish_free1/?cate_no=2-13"><span class="icon i7"></span>
		...
		<form action="<?=cdir()?>/dh/sidedish_free2" method="post" name="sendfrm" id="orderfrm">
		...
		<input type="hidden" name="cate_no" value="<?=$this->input->get('cate_no')?>">
			...
			<div class="order_sched_wrap">
				...
				<div class="info">
					...
					<span class="float-r">
						<button type="button" class="plain"><img src="/image/sub/btn_sched.png" alt="식단표 보기"></button>
					</span>
				</div><!-- END 안내 -->
