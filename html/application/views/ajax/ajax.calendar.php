...
?>
	...
	<table class="cm_cal clickable regmark">
		...
		<tbody>
				...
				for($col=0;$col<7;$col++){
					...
					if( $day > 0 ){
						...
						if($recom_max_date >= $today_is_nottoday and $today_is_nottoday > date("Y-m-d",$deliv_ok_date)){
							//배송시작일 기본값
							if( (strtotime($default_deliv_start_day) == strtotime($today_is_nottoday)) and in_array($week_name_arr[$today_day_name_val],$week_select_arr) and ( strtotime($today_is_nottoday) > time('+2 day') ) ){
								echo "start_deliv";
							}
							else{
						...
						?>"
							...
							data-yoil="<?=$default_deliv_start_day?>" data-date="<?=$today_is_nottoday?>"> <!-- 배송휴무 class명 "holiday" -->
			...
			<tr>
				<td colspan="7"  class="align-r pr10" style="height: auto;"><img src="/image/sub/cal01.png" alt="" style="height:20px;"></td>
			</tr>
