			...
			<tr>
				...
				for($col=0;$col<7;$col++){
					...
					if( $day > 0 ){
						...
						?>
						<td data-fus="<?if($_SERVER['REMOTE_ADDR']=="58.229.223.174"){ echo $today_date_format;print_r($holiday_arr); }?>" class="
							<?php
							$orderable = false;
							...
							?>">
							<a href="javascript: <?if($orderable){?>LoadFreefood('<?=$today_date_format?>')<?}else{?><?}?>;"><?=$day?></a>
						</td>
						<?php
					}
					else{
