	...
	<tbody>
					...
					<ul>
						...
						foreach($food_info as $info){
							...
							<li>
								...
								onchange="$('input[name=delivery_week_count]').removeAttr('checked');$('input[name=delivery_week_type]').removeAttr('checked');$('.info').hide();$('.sched_menu_box').html('<div class=no-ct>우측에서 주문설정을 하시면,<br>메뉴가 자동으로 표시됩니다</div>');">
		...
		<tr>
			<th>
				총 배송기간
				<div class="tooltip">
					<button type="button" class="plain btn" title="설명보기" name="deliv_adr03"><img src="/image/sub/icon_qmark.png" alt="설명보기"></button>
					...
					if($delivery_week_count){
						...
						<button type="button" onclick="toggleSelBox(this, event);">
							<span>
								...
								<strong> (<?=number_format($food_info[$delivery_week_day]['count'] * $delivery_week_count)?>팩)</strong>
								<?=number_format(($food_info[$delivery_week_day]['price_origin']+3000) * $delivery_week_count)?> →
								<strong><?=number_format( (($food_info[$delivery_week_day]['price_origin']+3000) * $delivery_week_count) - $food_info[$delivery_week_day]['delivery_week_count'][$delivery_week_count]['price'])?>원</strong>
								<em>(<?=$food_info[$delivery_week_day]['delivery_week_count'][$delivery_week_count]['price_per']?>% 할인)</em>
							</span>
						</button>
						<?php
						}
					...
					<ul>
						...
						if($search_idx){
							...
							foreach($food_info[$search_idx]['delivery_week_count'] as $info){
								...
								?>
								<li>
									<input type="radio" name="delivery_week_count" id="deliv_term<?=$i?>" value="<?php echo $info['val']; ?>" msg="1주~4주 배송받을기간을"
