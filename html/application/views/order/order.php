			...
			<div class="shop-wrap">
				...
				<div class="shop-order-wrap">
						...
						<table class="shop-cart">
							...
							<tbody>
								...
								foreach($cart_list as $lt){
									...
									if($frmCnt == 1){
										$pg_goods_name .= $lt->goods_name;
									}
									else{
										$pg_goods_cnt = ($frmCnt-1);
										$pg_goods_name .= " 외 {$pg_goods_cnt}건";
									}
									...
									<tr>
										...
										?>
										<td data-value="<?=$lt->date_bind?>">주 <?=$wt_arr[0]?>회<br><?=$wt_arr[1]?> 배송</td>
										<?php
										}
										else{
										?>
					...
					*/
					...
					<input type="hidden" name="relation_trade_code" value="">
					<input type="hidden" name="delivPoNm" value="<?=$readonly?"N":"";?>">

					<h3 class="order-tit">주문고객 정보</h3>
					<table class="order-field" <?if($_SERVER['HTTP_X_FORWARDED_FOR']=='58.229.223.174'){?>style="display:@none;"<?}?>>
					...
					<table class="order-field" <?if($_SERVER['HTTP_X_FORWARDED_FOR']=='58.229.223.174'){?>style="display:@none;"<?}?>>
						...
						<tbody>
							...
							<tr style="display:none;">
								<th>배송정보 선택</th>
								<td>
									<input type="radio" id="info-same" name="info-same" onclick="infoComp(1)" <?=($deliv_addr_set == "home")?"checked":"";?>><label for="info-same">주문고객 정보와 동일</label>
									<input type="radio" id="info-new" name="info-same" onclick="infoComp(2)" <?=($deliv_addr_set != "home")?"checked":"";?>><label for="info-new">주문고객 정보와 다름</label>
								</td>
							</tr>
							<tr style="display:none;">
								<th>도서산간지역 설정</th>
								<td><input type="checkbox" name="local_far" id="local_far" value="1">
									<small class="ml5"><label for="local_far">제주도 배송시에 체크해 주십시오. </label></small>
								</td>
							</tr>
							<tr>
								<th><label for="du-name">받으시는 분</label></th>
								<td><input type="text" id="du-name" name="send_name" msg="받으시는분을" value="<?=$rec_name?>" <?=$readonly?"readonly":"";?>></td>
							</tr>
							<tr>
								<th>주소</th>
								<td>
									<p>
										<label for="zipcode1" class="label-out">우편번호</label>
										<input type="text" class="field-s mr5" name="zip1" id="zipcode1" readonly msg="우편번호를" value="<?=$zip1?>">
										<?php
										if(!$readonly){
											?>
											<button type="button" class="btn-border-s" onclick="<?=$readonly?"alert('알림 : 배송비가 없는 주문은 배송지를 변경 하실 수 없습니다.')":"sample6_execDaumPostcode()";?>">우편번호찾기</button>
											<?php
										}
										?>
									</p>
									<p class="mt5">
										<input type="text" class="field-l" id="address1" name="addr1" readonly msg="주소를" value="<?=$addr1?>" style="width:400px;">
										<label for="address2" class="label-out">상세주소</label>
										<input type="text" class="field-l" id="address2" name="addr2" msg="상세주소를" value="<?=$addr2?>" <?=$readonly?"readonly":"";?>>
									</p>
								</td>
							</tr>
							<tr>
								...
								<td>
									<input type="text" class="field-xs" id="du-phone" name="send_phone1" maxlength="4" msg="받으시는분의 휴대폰번호를" value="<?=$rec_phone1?>" <?=$readonly?"readonly":"";?>> -
									<input type="text" class="field-xs" name="send_phone2" maxlength="4" msg="받으시는분의 휴대폰번호를" value="<?=$rec_phone2?>" <?=$readonly?"readonly":"";?>> -
									<input type="text" class="field-xs" name="send_phone3" maxlength="4" msg="받으시는분의 휴대폰번호를" value="<?=$rec_phone3?>" <?=$readonly?"readonly":"";?>>
								</td>
							</tr>
							<!-- <tr>
							...
							<tr>
								...
								<td><input type="text" name="send_text" class="field-xl" id="du-msg">
									<small class="ml15" style="display: inline-block;line-height: 1.4;vertical-align: -9px;">배송기사가 참고하는 내용으로 당사와 사전에 협의되지 않은 <br>지정일 및 제품, 배송 관련된 요청사항은 반영되지 않습니다.</small>
								</td>
							</tr>
							<tr>
								<th>택배사 안내</th>
								<td class="pt20 pb20"><img src="/image/sub/PC_order_03.jpg" alt=""><small style="display:inline-block;line-height: 1.4;" class="ml20">원활한 배송을 위해 우체국과 CJ택배를 병행하여 운영합니다. <br>우체국 이용고객도 CJ택배로 변경 될 수 있습니다.</small>
								</td>
							</tr>
						</tbody>
					</table>
					...
					<!-- 할인/결제 -->
					<div class="order-price-wrap" <?if($_SERVER['HTTP_X_FORWARDED_FOR']=='58.229.223.174'){?>style="display:@none;"<?}?>>
						<!-- 좌 -->
								...
								<tbody>
									...
									<tr class="coupon_block" style="display:none;">
										...
										<td>
											...
											<small class="ml15 bl-noti">사용 가능 쿠폰 : <strong class="em"><?=number_format($use_ok_coupon)?>장</strong></small>
										</td>
									</tr>
									<tr class="point_block" style="display:none;">
										...
										<td><input type="text" value="" id="use_point" class="field-s" maxlength="9" > P
											...
											<small class="ml15 bl-noti">보유포인트 : <strong class="em"><?=number_format($member_total_point)?> P</strong></small><br>
											<!-- <small class="">사용가능포인트(주문금액의 <?=$shop_info['point_percent']?>%) : <strong class="em"><?=number_format((($totalPrice+$delivery_price)/100)*$shop_info['point_percent'])?> P</strong></small> -->
										</td>
									</tr>
								</tbody>
							</table>
					...
					<!-- 총 주문금액 -->
					<div class="order-total-box" <?if($_SERVER['HTTP_X_FORWARDED_FOR']=='58.229.223.174'){?>style="display:@none;"<?}?>>
						<div class="each-price-box">
					...
					if($totalPrice+$delivery_price){
						...
						<table class="order-field mb0 pay_info" <?if($_SERVER['HTTP_X_FORWARDED_FOR']=='58.229.223.174'){?>style="display:@none;"<?}?>>
							...
							<tr>
								...
								<td>
									...
									<?php
									if($this->session->userdata('USERID') == 'test'){	//테스트계정만 보임
										?>
									...
									<input type="radio" name="trade_method" id="pay-way-dps" value="9"><label for="pay-way-dps">예치금</label>
									<input type="radio" name="trade_method" id="pay-way-transf" value="3"><label for="pay-way-transf">실시간 계좌이체</label>
									<input type="radio" name="trade_method" id="pay-way-phone" value="7"><label for="pay-way-phone">휴대폰결제</label>
									<?php
									if($this->session->userdata('USERID')=="test"){		//테스트계정만 보임
										?>
										<input type="radio" name="trade_method" id="pay-way-naver" value="22"><label for="pay-way-naver">네이버페이</label>
										<?php
									}
									?>
						...
						<!-- 카드결제 안내사항 -->
						<table class="order-field pay-info pay-way-card pay_info" id="pay-noti-card" <?if($_SERVER['HTTP_X_FORWARDED_FOR']=='58.229.223.174'){?>style="display:@none;"<?}?>>
						...
						<!-- 예치금 안내사항 -->
						<table class="order-field pay-info pay-way-dps pay_info" id="pay-noti-dps" style="display:none;">
							<caption>예치금 안내사항</caption>
							<tr>
								<th>사용 가능한 예치금</th>
						...
						<!-- 네이버페이 안내사항 -->
						<table class="order-field pay-info pay-way-naver pay_info" id="pay-noti-naver" style="display:none;">

							<caption>네이버페이 안내사항</caption>
							<tr>
								<td colspan="2">
									<p class="pay-info-tit mt15">네이버페이 안내사항</p>
									<ul class="order-noti">
						...
						<!-- 휴대폰결제 안내사항 -->
						<table class="order-field pay-info pay-way-phone pay_info" id="pay-way-phone" style="display:none;">
							<!-- <caption>가상계좌 안내사항</caption>
							<tr>
								<td colspan="2">
									<p class="mt5">가상계좌 결제 시 화면 아래 '결제하기'버튼을 클릭하시면 가상계좌 결제 창이 나타납니다.</p>
						...
						<!-- <input type="radio" name="trade_method" checked value="99" style="display:none;">
				...
				<div class="align-c mt15">
					...
					<?php
						...
						if($order_confirm){
							?>
							<button type="button" class="btn-emp-border" onclick="location.href='<?=$_SERVER['HTTP_REFERER']?>'">이전 페이지로</button>
							<button type="button" class="btn-emp send_order" name="writeBtn">결제하기</button>
							<?php
						}
						else{
							?>
							<button type="button" class="btn-emp" name="writeBtn" onclick="alert('배송일이 지난 상품은 주문할 수 없습니다.\n<?=date("Y년 m월 d일",strtotime($order_cant_date))?> 이전으로\n배송일이 지정된 상품은 주문할 수 없습니다.')">결제하기</button>
							<?php
						}
					}
					?>
...
<script>
	function addr_change(val){
		$.ajax({
			url:"/html/dh_order/addr_change/?type="+val+"&userid=<?=$this->session->userdata('USERID')?>",
			type:"GET",
			dataType:"json",
			cache:false,
			error:function(xhr){
	...
	$(function(){
		$(".shop-order-wrap table tbody").rowspan(6);
...
<iframe name="tmp_frame" border=0 frameborder=0 style="display:none;width:100%;height;1500px;"></iframe>
