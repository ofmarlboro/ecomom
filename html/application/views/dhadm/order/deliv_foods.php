...
<html lang="ko" style="overflow-x:hidden;overflow-y:scroll;">
 ...
 <body>
	...
	<h1 style="font-size:20px;font-weight:600;margin:20px">배송일별 식단정보 ( <?=$this->input->get('code')?> )</h1>
	...
	<table class="adm-table">
		...
		$prod_cnt = 0;
		foreach($list as $lt){
			$prod_cnt+=$lt->prod_cnt;
		?>
		<tr style="background:<?=($lt->prod_cnt <= 0)?"gray":"";?>;">
			<td data-value="<?=strtotime($lt->deliv_date)?>"><?=$lt->deliv_date?> (<?=numberToWeekname($lt->deliv_date)?>)</td>
			<td>
				<?php
				if($lt->list_img){
				?>
				<img src="/_data/file/goodsImages/<?=$lt->list_img?>" style="max-width:30px;vertical-align:middle">
				<?php
				}
