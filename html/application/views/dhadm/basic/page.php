...
?>
				...
				<table class="adm-table">
					...
					<tbody>			
						<tr>
							...
							<td>
								<select name="item">
									<option value="title" <?if($this->input->get('item')=="title"){?>selected<?}?>>타이틀</option>
									<option value="page_index" <?if($this->input->get('item')=="page_index"){?>selected<?}?>>Page Index</option>
								</select>
				...
				<table class="adm-table line align-c">
					...
					<tbody class="ft092">
						...
						<tr>
							<td><?=$totalCnt?></td>
							<td><a href="<?=self_url()?>/view/<?=$lt->idx?>/<?=$query_string.$param?>"><?=$lt->title?></a></td>
							<td>
				...
				<div class="float-wrap mt20">
					<div class="float-r">
						<a href="<?=self_url()?>/write/" class="button btn-ok">페이지 등록</a></span>
					</div>
				</div><!-- END 제품 액션 버튼 -->
