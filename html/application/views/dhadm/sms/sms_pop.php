...
<div id="phone_body" style="margin-left: 125px;">
  <!-- <form id="MsgForm" name="MsgForm" method="POST" action="<?=cdir()?>/board/sms_send" onSubmit="return varcheck();"> -->
  <form id="MsgForm" name="MsgForm" method="POST" action="<?=cdir()?>/member/sms_send/?ajax=1" onSubmit="return varcheck();">
  ...
  <div class="Numb">
      ...
      <span class="button2"><a href="javascript:delItem()">삭제</a></span>
			...
			*/
			?>
      <select name="strDest" size="10" multiple class="sendNumb" id="phonenos">
      <!-- <select name="send_phone" multiple style="width:100%;height:100px;"> -->
				<option value="0">--수신자--</option>
				<option value="<?=$phone?>@@<?=$name?>@@<?=$userid?>"><?=$phone?></option>
      </select>
      보낸번호 <input type="text" name="strCallBack" value="0558842625">
  </div>
  <!-- //받는번호 보내는번호 -->
  <div class="line"></div>
  <!-- 즉시전송 예약전송선택 -->
  <div class="Rn">
      ...
      <tr>
          ...
          <td style="width:50%;display:none;" class="ac"><input name="chkSendFlag" type="radio" value="1" onClick="CWCheck(this.form)"><div class="rt">예약전송</div></td>
      </tr>
      </table>
  </div>
  ...
  <div class="btnarea">
      ...
      <!-- <a href="javascript:;" onclick="varcheck()"><img src="/sms_images/btn_send.gif" alt="문자전송"></a> -->
      <a href="javascript:;" onclick="reset();"><img src="/sms_images/btn_reset.gif" alt="다시쓰기"></a>
  </div>
  <!-- //발송버튼 -->
  </form>
</div>