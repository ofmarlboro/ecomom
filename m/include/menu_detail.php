<script type="text/javascript">
	function menuView(idx,recom_is,deliv_date,price,name,origin_price){
		$.ajax({
			type:"GET"
			,url:"<?=cdir()?>/dh/dh_ajax/?ajax=1&mode=get_prod_view&goods_idx="+idx+"&recom_is="+recom_is+"&deliv_date="+deliv_date+"&price="+price+"&name="+encodeURIComponent(name)+"&origin_price="+origin_price
			,dataType:"json"
			,cache:false
			,error:function(xhr){
...
<div id="menu_dt_wrap" style="display:none;">
	...
	<div id="menu_dt">
		...
		<div class="scroll">
			...
			<?php
			if(strpos($refer,"free_list")!==false){
			?>
			<p class="mt40 align-c">
				<button type="button" class="plain" onclick="location.href='<?=cdir()?>/dh/free_list/?cate_no=<?=$row->cate_no?>'"><img src="/image/sub/btn_order2.jpg" alt="주문하기"></button>
			</p>
			<?php
			}
			else if($this->input->get('recom_idx')){
			?>
			<p class="mt40 align-c">
				<button type="button" class="plain" onclick="location.href='<?=cdir()?>/dh/regular01/?recom_idx=<?=$this->input->get('recom_idx')?>'"><img src="/image/sub/btn_order2.jpg" alt="주문하기"></button>
			</p>
			<?php
			}
			else{
			?>
			<p class="mt40 align-c">
				<button type="button" class="plain" onclick="location.href='<?=cdir()?>/dh/sale_list'"><img src="/image/sub/btn_order2.jpg" alt="주문하기"></button>
			</p>
			<?php
			}
			?>
