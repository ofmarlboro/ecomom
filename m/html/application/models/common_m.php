...
class Common_m extends CI_Model
{
	...
	public function mailform($item, $data='')
	{
		...
		if($email){
			...
			*/
			$result = $this->sendEmail_cifunc($name,$email,$from_name,$from_email,$title,$to_content);
	...
	*/
	...
	public function sendEmail($toname,$tomail,$fromname,$frommail, $subject, $message) {
		...
		$flag = mail($tomail, $title, $message, $mailHeader, $frommail);
		return $flag;
		...
		public function auto_login(){
			$auto_login = $this->input->cookie(md5("auto_login"));
			$auto_login_uid = $this->input->cookie(md5("auto_login_uid"));

			if($auto_login && $auto_login_uid){

				$decode_auto_login_uid = base64_decode($auto_login_uid);
				$row = $this->self_q("select * from dh_member where userid = '{$decode_auto_login_uid}'","row");
				$userid = $row->userid;
				$passwd = $row->passwd;
				$name = $row->name;
				$level = $row->level;

				$rel_existence = md5($_SERVER['HTTP_HOST'] . $row->passwd);
				$tmp_existence = base64_decode($auto_login);

				if($tmp_existence and $tmp_existence == $rel_existence){
