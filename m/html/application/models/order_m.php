...
class Order_m extends CI_Model
{
	...
	public function cart(){
		...
		public function trade_tmp_add($trade_code,$data='')
		{
			...
			if( $result ){ //무통장입금 or 포인트결제 or 쿠폰결제 일경우
			...
			if($tmp_cnt){
				...
				//trade 등록 start
					...
					foreach($holi as $h){
						...
						//					$insert_array = array(		//DB 입력값들
						//						'trade_code'=>$trade_code,
						//						'trade_stat'=>$trade_stat ,
						//						'trade_method'=>$trade_data['trade_method'],
						//						'trade_day'=>date("Y-m-d H:i:s"),
						//						'trade_day_ok'=>$trade_day_ok,
						//						'userid'=>$trade_data['userid'],
						//						'name'=>$trade_data['name'],
						//						'phone'=>$trade_data['phone'],
						//						'email'=>$trade_data['email'],
						//						'send_name'=>$trade_data['send_name'],
						//						'send_phone'=>$trade_data['send_phone'],
						//						'send_tel'=>$trade_data['send_tel'],
						//						'zip1'=>$trade_data['zip1'],
						//						'addr1'=>$trade_data['addr1'],
						//						'addr2'=>$trade_data['addr2'],
						//						'send_text'=>$trade_data['send_text'],
						//						'save_point'=>$trade_data['save_point'],
						//						'use_point'=>$trade_data['use_point'],
						//						'coupon_idx'=>$trade_data['coupon_idx'],
						//						'use_coupon'=>$trade_data['use_coupon'],
						//						'total_price'=>$trade_data['total_price'],
						//						'mobile'=>$trade_data['mobile'],
						//						'price'=>$trade_data['price'],
						//						'enter_name'=>$trade_data['enter_name'],
						//						'enter_bank'=>$trade_data['enter_bank'],
						//						'enter_account'=>$trade_data['enter_account'],
						//						'enter_info'=>$trade_data['enter_info'],
						//						'cash_receipt'=>$trade_data['cash_receipt'],
						//						'cash_number'=>$trade_data['cash_number'],
						//						'goods_price'=>$trade_data['goods_price'],
						//						'delivery_price'=>$trade_data['delivery_price'],
						//						'local_far'=>$trade_data['local_far'],
						//						'tno'=>$tno,
						//						'first_order'=>$first_order,
						//						'cash_authno'=>$this->input->post("cash_authno"),
						//						'point_pay'=>$trade_data['point_pay']
						//					);
						...
						*/
						...
						foreach($data['cart_list'] as $lt){
							...
							if($lt->recom_is == "Y"){	//추천식단일 경우
								...
								$insert_data['deliv_price'] = $lt->deliv_price;
	...
	public function Epost_deliv_Api($deliv_code, $reqType=1){	//송장번호 자동업데이트
		...
		$epost_sql = "select distinct d.order_name, m.zip1, m.add1, m.add2, m.phone1, m.phone2, m.phone3, m.tel1, m.tel2, m.tel3,
										...
										d.prod_name, d.trade_code, t.send_text, sum(p.prod_cnt) as qty
		...
		$data['ordAddr1'] = $epost_result->add1;
		$data['ordAddr2'] = $epost_result->add2;
		$data['ordTel'] = $epost_result->tel1.$epost_result->tel2.$epost_result->tel3;
		...
		$data['recAddr1'] = $epost_result->addr1;
	...
	public function uplus_pay_Gethash($post){
			...
			require_once $_SERVER['DOCUMENT_ROOT']."/pay/uplus/m/lgdacom/XPayClient.php";
	...
	public function uplus_pay($post){
		...
		?>
		<form method="post" name="LGD_RETURNINFO" id="LGD_RETURNINFO" action="<?=cdir()?>/dh_order/uplus_pay_ok">
		<?php
	...
	public function grade_pay($post){
		...
		?>
		<form method="post" name="LGD_RETURNINFO" id="LGD_RETURNINFO" action="<?=cdir()?>/dh_order/grade_pay_ok">
		<?php
				foreach ($payReqMap as $key => $value) {
					echo "<input type='hidden' name='$key' id='$key' value='$value'>";
				}
	...
	public function uplus_pay_ok($post){
		...
		if($xpay->TX()){
			...
			if( "0000" == $xpay->Response_Code() ) {
				...
				alert("/m/","통신상의 문제로 인하여 결제요청이 실패하였습니다.");
			}
			...
			//2)API 요청실패 화면처리
			alert("/m/","API 요청실패로 인하여 결제요청이 실패하였습니다.");
	...
	public function grade_pay_ok($post){
		...
		if($xpay->TX()){
			...
			if( "0000" == $xpay->Response_Code() ) {
				...
				if( !$isDBOK ) {
					...
					$tmp = $this->common_m->self_q("select * from dh_grade_chg_tmp where change_trade_code = '{$lgd_oid}'","row");
					$change_tmp = unserialize($tmp->change_tmp);

					?>
					<form action="<?=cdir()?>/dh_order/order_edit02/?mode=grade_change" method="post" name="pay_comp" id="pay_comp">
						<?php
						foreach($change_tmp as $k=>$v){
						?>
						<input type="hidden" name="<?=$k?>" value="<?=$v?>">
						<?php
						}
	...
	public function vacctinput_m()
	{
		...
		//118.129.210.25, 183.109.71.153, 203.238.37.15
		$PG_IP = array('183.109.71.153','118.129.210.25','203.238.37.15');
		...
		if(in_array($TEMP_IP,$PG_IP))	//PG에서 보냈는지 IP로 체크
		{
			...
			if($P_TYPE == "BANK"){
				...
				$noti_in['trade_code'] = $P_OID;
				$noti_in['noti'] = $noti;
				$noti_in['wdate'] = timenow();

				$this->common_m->insert2("dh_inicis_dbank",$noti_in);
			}
			...
			if($P_TYPE == "VBANK"){	//결제수단이 가상계좌이며
				if($P_STATUS != "02"){ //입금통보 "02" 가 아니면(가상계좌 채번 : 00 또는 01 경우)
					...
					/*
					$trade_stat = $this->common_m->getRow("dh_trade","where trade_code='{$P_OID}'");
					if($trade_stat->trade_stat==1 && $trade_stat->trade_method==4 ){ //입금대기 && 가상계좌일경우	//&& $trade_stat->tno == $P_TID
						$dbOk = $this->common_m->update2("dh_trade",array('trade_stat'=>2,'trade_day_ok'=>date("Y-m-d H:i:s")),array('idx'=>$trade_stat->idx)); //결제완료로 변경
						if($dbOk){ $dbOk = "OK"; }
					}
					else{
				...
				$num_rows = $this->common_m->self_q("select * from dh_trade where trade_code = '".$P_OID."'","cnt");
				if($num_rows){
					$dbOk = "OK";
				}
			}
	...
	public function inicis_dbank_return(){

		$trade_code = $this->input->get('P_OID');
		$trade_idx = $this->input->get('P_NOTI');

		$inicis_bankrow = $this->common_m->self_q("select * from dh_inicis_dbank where trade_code = '{$trade_code}'","row");
		$noti = explode("^^",$inicis_bankrow->noti);

		foreach($noti as $val){
			$real_noti = explode("=",$val);
			${$real_noti[0]} = $real_noti[1];
		}

		$sql = "update dh_trade set trade_stat = 2, trade_day_ok = now(), tno = '".$P_TID."' where trade_code = '".$P_OID."'";
		$result = $this->common_m->self_q($sql,"update");
		if($result){
			?>
			<body onload="javascript:document.order_form.submit();">
			<form name="order_form" id="order_form" method="post" action="<?=cdir()?>/dh_order/shop_order/<?=$trade_idx?>">
				<input type="hidden" name="tno" id="tno" value="<?=$P_TID?>">
				<input type="hidden" name="ResultCode" value="<?=($P_STATUS == "00")?"0000":"";?>">
				<input type="hidden" name="ResultMsg" value="결제가 정상 처리 되었습니다.">
				<input type="hidden" name="PayMethod" value="<?=$P_TYPE?>">
				<input type="hidden" name="trade_ok" value="1">
