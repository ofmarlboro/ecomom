	...
	<?
		...
		if(isset($query_string) && isset($param)){
			$send.=$query_string.$param."&ajax=1&goods_idx=".$goods_row->idx;
		}else{
			$send= "?";
		}
	...
	?>
				<div class="float-wrap mb20">
					<h4 class="float-l tab-tit">제품리뷰 작성하기</h4>
				</div>
				...
				<input type="hidden" name="goods_idx" value="<?=$goods_row->idx?>">
				<input type="hidden" name="cate_no" value="<?=$goods_row->cate_no?>">

				<!-- board write -->
				<div class="board-write mb50" id="write_review" >
					<ul class="board-write-form">
						<li>
							<div class="field-label">구매상품</div>
							<div class="field-form">[<?=$goods_row->name?>]</div>
						</li>
					...
					<div class="board-view-btns">
						<!-- !!!! 작성한 모든 값을 reset !!!! 하고 write form을 닫음. -->
						<input type="button" class="btn-border" value="취소" onclick="bbs_load('<?=$bbs->code?>')">
						<input type="button" class="btn-emp" value="등록하기" value="<? if($this->uri->segment(2)=="write"){?>등록<?}else{?>수정<?}?>하기" onclick="javascript:validForm();">
					</div><!-- END Buttons -->
