			...
			$.ajax({
				...
				,success:function(data){
					...
					if(data.prod_list){
						...
						$("span.set").text(data.price_info.pack_ea+"팩("+data.price_info.per+"%)");
				...
				<div class="selbox selbox02">
					...
					if( strpos($_SERVER['REQUEST_URI'],"prod_list")===false and $this->input->get('recom_idx') ){
						...
						<ul>
							...
							<li class="pe04"><input type="radio" id="pe2" onclick="location.href='/m/html/dh/bfood_order_free1/?cate_no=1-10'"><label for="pe2"><span></span><strong>9~12개월 </strong> 후기</label></li>
							...
							<li class="pe03"><input type="radio" id="pe7" onclick="location.href='/m/html/dh/bfood_order_free1/?cate_no=1-9'"><label for="pe7"><span></span><strong>7~8개월 </strong> 중기</label></li>
						...
						if($SubName=="K0704"){	//산골야시장
							...
							if($cate_stat1->title){
								...
								if($PageName == "SALE_LIST"){
									?>
									<button type="button" style="background:#BCA061;"><span class="week_day_count_span">특가상품팩</span></button>
									<?php
								}
							}
						}
...
<!--   <div class="bottom_bar bottom_bar03 bottom_bar03_1">
  ...
  <p class="align-c">
        ...
        if(!$this->session->userdata('USERID')){
          ...
          ?>
          <button type="button" class="plain orderbtn" title="주문하기" onclick="frmChk('recomfrm','regular_order_test')"><img src="/m/image/sub/order_0_03.png" alt="주문하기">주문하기</button>
          <?php
				}
				else{
          if($recom_overlap){
