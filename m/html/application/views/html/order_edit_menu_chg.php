...
<head>
	...
	<style type="text/css">
		h3.on{font-size: 16px;background: #105F5F; color: #fff; text-align: center; font-weight: bold; line-height: 40px;border:5px solid #ff9900;}
	</style>
...
<body>
	<div class="not_layer_pop pt20">
	<form method="post" name="menu_cnt_chg" id="menu_cnt_chg">
		<div class="inner">
			<h1>
				<span class="pop_bag" style="margin-right:10px">메뉴변경</span><br><?=date("m/d",strtotime($row->deliv_date))?>(<?=numberToWeekname($row->deliv_date)?>) 배송 식단 변경
						
				<div style="font-size: 17px;">	
					<span>주문팩수 : </span>
					<!-- <span><?=$sum_foods->total?></span> / <span><?=$sum_foods->total?></span>팩 -->
					<input type="text" name="chang_cnt" value="<?=$sum_foods->total?>" style="width:25px;border:none;" readonly> /
					<input type="text" name="stand_cnt" value="<?=$sum_foods->total?>" style="width:25px;border:none;" readonly>팩
				</div>	
			</h1>
			<div class="inner clearfix">
				...
				<ul class="cart_menu clearfix cart_menu01">
					<?php
					if($foods_list){
						$list_cnt = 0;
			...
			<button type="button" class="btn_y" title="닫기" onclick='self.close()'>취소</button>
				<button type="button" class="btn_y" title="닫기" onclick="send_form()">변경</button>
			</div>
