...
class Dh_order extends CI_Controller {
	...
	{
		$this->common_m->auto_login();
		$data['shop_info'] = $this->common_m->shop_info(); //shop 정보
		$data['cate_data'] = $this->product_m->header_cate(); //헤더에 보여질 모든 카테고리 리스트
		$data['cart_cnt_hd'] = 0;
		if($this->session->userdata('USERID')){
			$data['cart_cnt_hd'] = $this->common_m->self_q("select * from dh_cart where trade_ok != 1 and (userid = '".$this->session->userdata('USERID')."' or code = '".$this->session->userdata('CART')."')","cnt");
			$data['member_info'] = $this->common_m->self_q("select * from dh_member where userid = '".$this->session->userdata('USERID')."'","row");
		}

		$level_row = $this->common_m->self_q("select * from dh_member_level order by level asc","result");
		$data['member_level'] = array();
		foreach($level_row as $lr){
			$data['member_level'][$lr->level] = $lr->name;
		}
	...
	public function shop_order($data='')
	{
		...
		if($sample_is == "ok"){
			...
			//			foreach($data['cart_list'] as $lt){
			//				$deliv_date = strtotime($lt->date_bind);
			//				if($deliv_date < $limit_day){
			//					back("배송가능한 날짜가 아닙니다. 장바구니를 확인해 주세요.");
			//				}
			//			}
			...
			**************************************************************************************/
			...
			//			if(count($data['cart_list']) > 1){
			//				foreach($data['cart_list'] as $lt){
			//
			//					//의기양양픽 주문시 합배송 불가하게 처리
			//					if($lt->goods_idx == "585" || $lt->goods_idx == "584"){
			//						$this->common_m->self_q("delete from dh_cart where trade_ok <> 1 and goods_idx in (585,584) and userid = '{$USERID}'","delete");
		...
		*/
		...
		//
		//		$where_query = str_replace("c.","",$where_query);
		//
		//		$data['totalPrice'] = $this->common_m->getSum($db_table,"total_price","where trade_ok!='1' $where_query ");
		//		$data['totalCnt'] = $this->common_m->self_q("select * from {$db_table} where 1 {$where_query}","cnt");
		...
		/*
		if($data['totalCnt']==0){
			$pg_row = $this->common_m->self_q("select * from dh_uplus_pay where trade_code = '".$this->input->post("trade_code")."'","row");
			$pg_result = unserialize($pg_row->result_arr);
			//카드결제 성공후 DB 입력이 실패하여 DB에 입력이 안될경우 카드결제 취소시킨다
			?>
			<form method="post" name=ini id="LGD_PAYINFO" action="/pay/uplus/Cancel.php">
				<input type="hidden" name="CST_MID" value="<?=$data['shop_info']['pg_id']?>"/>
				<input type="hidden" name="CST_PLATFORM" value="<?=($data['shop_info']['lgu_test'] == "ok")?"test":"service";?>"/>
				<input type="hidden" name="LGD_TID" value="<?=$pg_result['LGD_TID']?>">
				<input type="hidden" name="go_url" value="/html/dh/shop_order_error">
			</form>
		...
		if($data['totalCnt']==1){ //단일상품일때 상품정책
			...
			$cart_stat = $this->common_m->getRow3("{$db_table} c, dh_goods g","where c.goods_idx=g.idx $where","g.express_no_basic,g.express_check,g.express_money,g.express_free");
		...
		//$data['delivery_price'] = ($sample_is == "ok") ? "0" : $data['delivery_price'] ;
		*/

		//에ㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔㅔ코ㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗㅗ맘ㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁ
		...
		//				$a_idx_arr = explode("a",$a_idx);
		//				if(count($a_idx_arr)==1){
		//					$deliv_where .= " and idx='".$this->db->escape_str($a_idx)."'";
		//				}else if(count($a_idx_arr) > 1){
		//
		//					$deliv_where .= " and (";
		//					for($i=0;$i<count($a_idx_arr);$i++){
		...
		//				from dh_cart
		//				where userid = '{$USERID}' and trade_ok != '1' and goods_idx not in ('485','543','505','554')
		//				{$deliv_where}
		//				group by date_bind
		//			";
		//
		//			$deliv_add_yn = $this->common_m->self_q($deliv_addyn_sql,"result");
		//
		//			//pr($deliv_add_yn);
		//			//pr($db_dv_date_arr);
		//
		...
		//
		//				$gontong_where = " and date_bind = '".$dayn->date_bind."'";
		//				$deliv_where .= $gontong_where;
		//
		//				if($dayn->group_sum_price >= $data['shop_info']['express_free'] or in_array($dayn->date_bind,$db_dv_date_arr) or $dayn->recom_is > 0){
		//					$result = $this->common_m->self_q("update dh_cart set deliv_price = '0' where trade_ok <> 1 and userid = '".$dayn->userid."' {$deliv_where}","update");
		//				}
		//				else{
		//					$data['delivery_price'] += $data['shop_info']['express_money'];
		//					$result = $this->common_m->self_q("update dh_cart set deliv_price = '".$data['shop_info']['express_money']."' where trade_ok <> 1 and userid = '".$dayn->userid."' {$deliv_where}","update");
		//
		//				}
		//			}
		//
	...
	public function shop_cart($data=''){
		...
		if($sample_is == "ok"){	//샘플신청시
			...
			*/
			...
			//					if($hdl->goods_idx == "585" || $hdl->goods_idx == "584"){
			//						$this->common_m->self_q("delete from dh_cart where trade_ok <> 1 and goods_idx in (585,584) and userid = '{$USERID}'","delete");
			//						alert("/m/html/dh_order/shop_cart","합배송이 불가한 제품이 있어 장바구니에서 삭제하였습니다.");
			//					}
			//				}
			//			}
			//
			...
			**************************************************************************************/
			...
			if($cnt > 1){
				$result = $this->common_m->self_q("delete from dh_cart where userid = '".$this->session->userdata('USERID')."' and goods_idx = '855'","delete");
				if($result){
					alert('/',"의기양양픽은 1개만 주문가능합니다.");
				}
			}
			...
			*/
			...
			$data['delivery_price'] = 0;
			$deliv_add_yn = $this->common_m->self_q("select date_bind , sum(total_price) as group_sum_price, max(deliv_price) as deliv_price from dh_cart where userid = '{$USERID}' and trade_ok != '1' group by date_bind","result");
	...
	*/
	...
	public function shop_order_detail($data='')
	{
		...
		if($trade_cnt > 0){
			...
			$deliv_list_sql = "
				select a.*, b.total_price, (select name from dh_goods where idx = a.goods_idx) as goods_name
	...
	public function uplus_paytmp($data=''){
		...
		if($result){
			//echo "ok";
			script_exe('parent.launchCrossPlatform();');
		}
	}
	...
	public function grade_pay($data=''){
		$this->order_m->grade_pay($_POST);
	}
	...
	public function grade_pay_ok($data=''){
		$this->order_m->grade_pay_ok($_POST);
	}
	...
	public function recom_cart($data=''){
		...
		//					}else{
		//						if($lt->trade_code){
		//							alert($_SERVER['HTTP_REFERER'],"년간 1회만 주문가능합니다.");
		//							/*
		...
		if($recom){	//추천식단 장바구니 담기
			...
			foreach($recom_delivery_detail_date_arr as $deliv_date_time){	//식단정보 배열처리
				...
				foreach($prod_cnt as $k=>$pc){
					...
					//알러지체크후 제품 변경여부 확인
					$recom_food_table[$deliv_date_time][$k]['allergy'] = $this->input->post("alg_chg_cnt_".$deliv_date_time) > 0 ? $this->input->post("alg_chg_cnt_".$deliv_date_time) : "0" ;
				}
			}
		...
		alert(cdir()."/dh_order/shop_cart");
		//echo "<script>location.replace('".cdir()."/dh_order/shop_cart')</script>";
	...
	public function recom_food_list($data=''){	//장바구니 식단보기 팝업
		...
		$goods = $this->common_m->self_q("select * from dh_goods order by name asc","result");
		$goods_info = array();
		foreach($goods as $gd){
	...
	public function get_Deliv_prod_pop($deliv_code){
		...
		$list = $this->common_m->self_q("select a.*, b.name from dh_trade_deliv_prod a, dh_goods b where a.goods_idx = b.idx and deliv_code = '{$deliv_code}' order by recom_idx desc","result");
			...
			if($mode == "deliv_addr_chg"){	//팝업
				...
				if($_POST){	//폼값이 전달 되었을때
					...
					if($this->input->post('check') == 'all'){	//이후부터 일괄
						...
						foreach($list as $lt){
							$result = $this->common_m->insert_log($userid,timenow(),'배송지 변경','배송지를 변경하였습니다.',$lt->deliv_code,'본인');
						}
						...
						$result = $this->common_m->insert_log($userid,timenow(),'배송지 변경','배송지를 변경하였습니다.',$this->input->post('deliv_code'),'본인');
					...
					if($result){	//업데이트 성공시
						...
						<script type="text/javascript">
						<!--
							alert('배송지 변경이 완료되었습니다.\n\n변경내용은 [배송지변경]을 요청주신 페이지를 통해서만 확인가능합니다.');
					...
					$this->load->view("/html/order_edit_delivaddr_chg",$data);
				}
		...
		if($userid){	//세션체크
			...
			if($mode == "menu_change"){	//메뉴변경
				...
				if($_POST){	//변경 폼 전달받은뒤
					...
					//알러지로 메뉴 변경시 오더타입 변경 및 알러지체크 표기
				...
				if($_POST){
					...
					if($this->input->post('submode') == "change_tmp"){

						$change_tmp = array();

						foreach($_POST as $key=>$value){
							if($key != "submode"){
								$change_tmp[$key] = $value;
							}
						}

						$insert_data['change_trade_code'] = $this->input->post('change_trade_code');
						$insert_data['cart_id'] = $this->input->post('cart_id');
						$insert_data['userid'] = $this->input->post('userid');
						$insert_data['change_tmp'] = serialize($change_tmp);
						$insert_data['wdate'] = timenow();

						$result = $this->common_m->insert2("dh_grade_chg_tmp",$insert_data);
						if($result){
							echo "ok";
						}
				...
				$data['reverse_recom_date'] = $recom_date;
				$data['deliv_start_date'] = date("Y-m-d",$deliv_code_arr[1]);
				...
				if($_POST){
					...
					*/
					...
					$remain_pack_sql = "
						...
						and (select deliv_stat from dh_trade_deliv_info where deliv_code = dh_trade_deliv_prod.deliv_code) in (0,6)
					";
	...
	*/
	...
	public function deliv_addr_list_change(){	//배송지변경 리스트에서 셀렉트값 변경시 주소값도 같이 변경
		...
		echo '<span class="blue"> #'.DelivAddrName($row->deliv_addr).'<br></span>'.$row->addr1;
	...
	public function change_grade(){	//단계변경 단계선택시 차액 계산
		...
		*/
		...
		//list($trade_code,$deliv_date_time) = explode("-",$deliv_code);	//배송코드를 주문코드와 날짜타임으로 반환
	...
	public function cancel_list($data=''){
		...
		if($userid){
			/*
			$sql = "select distinct b.*, a.recom_dates, a.recom_is, a.sample_is, a.grade_change, a.trade_stat, a.trade_method
							, (select grade_change_recom_name from dh_trade_goods where trade_code = a.trade_code and cate_no = 'recom') as grade_change_recom_name
	...
	public function deliv_chart(){
		$trade_code = $this->input->get('trade_code');
		$deliv_info = $this->common_m->self_q("select * from dh_trade_deliv_info where trade_code = '{$trade_code}' order by deliv_date asc","result");
		$deliv_stat_arr = array('0'=>'배송대기','1'=>'배송준비중','2'=>'배송중','3'=>'배송완료','4'=>'중지','5'=>'취소','6'=>'배송휴무');
		...
		<div class="layer_pop_inner02">
					...
					foreach($recom_days_n_stat_arr as $rdnsa){
						...
						foreach($rdnsa as $key=>$rd){
							if($key != "prod_name"){
								${'dcnt'.$rdnsa_cnt}++;
								?>
								<tr><th><?=${'dcnt'.$rdnsa_cnt}?>회차</th><td><?=$rd['date']?></td><td><?=$deliv_stat_arr[$rd['deliv_stat']]?></td></tr>
								<?php
							}
						}
						?>
			...
			<button type="button" class="w100 close" title="닫기" onclick='closedeliv_daylist();'>확인</button>
		</div>
		<?php
	}
	...
	public function pg_vali(){
		$oid = $this->input->get('oid');
		$sql = "select * from dh_trade_tmp where trade_code = '{$oid}'";
		$result = $this->common_m->self_q($sql,"cnt");
		echo $result;
	}
	...
	public function inicis_dbank_return($data='') //이니시스 모바일 실시간계좌이체
	{
		$result = $this->order_m->inicis_dbank_return();
		echo $result;
	}
	...
	public function deposit_order(){
		$ini_arr['TID'] = $_POST['tno'];
		$ini_arr['ResultCode'] = $_POST['ResultCode'];
		$ini_arr['ResultMsg'] = $_POST['ResultMsg'];
		$ini_arr['PayMethod'] = $_POST['PayMethod'];
		$ini_arr['VACT_Num'] = $_POST['VACT_Num'];
		$ini_arr['VACT_BankCode'] = $_POST['VACT_BankCode'];
		$ini_arr['VACT_Date'] = $_POST['VACT_Date'];
		$ini_arr['VACT_InputName'] = $_POST['VACT_InputName'];
		$ini_arr['VACT_Name'] = $_POST['VACT_Name'];
		$ini_arr['MOID'] = $_POST['trade_code'];
		$ini_arr['TotPrice'] = $_POST['total_price'];
		$ini_arr['regDate'] = timenow();

		$result = $this->common_m->insert2("dh_inicis_pay",$ini_arr);

		if($result){
			$tmpRow = $this->common_m->getRow2("dh_trade_tmp","where trade_code='".$_POST['trade_code']."' order by idx limit 1");
			$trade_data = decode($tmpRow->data);

			$update_data['trade_code'] = $ini_arr['MOID'];
			$update_data['trade_stat'] = 1;
			$update_data['trade_method'] = 4;
			$update_data['trade_day'] = $ini_arr['regDate'];
			$update_data['userid'] = $trade_data['userid'];
			$update_data['name'] = $trade_data["P_UNAME"];
			$update_data['phone'] = $trade_data["P_MOBILE"];
			$update_data['email'] = $trade_data["P_EMAIL"];
			$update_data['send_name'] = $trade_data["P_UNAME"];
			$update_data['send_phone'] = $trade_data["P_MOBILE"];
			$update_data['total_price'] = $ini_arr['TotPrice'];
			$update_data['price'] = $ini_arr['TotPrice'];
			$update_data['goods_price'] = $ini_arr['TotPrice'];

			$update_data['newzip'] = 'deposit';

			$update_data['enter_name'] = $ini_arr['VACT_InputName'];
			$update_data['enter_bank'] = inicis_bank_array($ini_arr['VACT_BankCode']);
			$update_data['enter_account'] = $ini_arr['VACT_Num'];
			$update_data['enter_info'] = $ini_arr['VACT_Name'];

			$update_data['tno'] = $ini_arr['TID'];

			$result = $this->common_m->insert2("dh_trade",$update_data);

			if($result){
				$table = "dh_trade_goods";

				$insert_data['trade_code'] = $ini_arr['MOID'];
				$insert_data['cate_no'] = "deposit";
				$insert_data['goods_name'] = $trade_data["P_GOODS"];
				$insert_data['total_price'] = $ini_arr["TotPrice"];
				$insert_data['goods_price'] = $ini_arr["TotPrice"];
				$insert_data['goods_cnt'] = 1;
				$insert_data['trade_day'] = $ini_arr['regDate'];

				$result = $this->common_m->insert2($table,$insert_data);
				if($result){
					alert(cdir()."/dh_order/deposit_ok/".$ini_arr['MOID']);
				}
			}
		}
	}
