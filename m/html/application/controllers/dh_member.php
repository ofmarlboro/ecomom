...
class Dh_Member extends CI_Controller {
	...
	public function login($data='')
	{
		if($this->session->userdata('USERID')){
			...
			if($userid && $passwd){ //로그인
				...
				if($result && $result->di == ""){
					alert(cdir()."/dh/identity","회원정책 변경으로 인하여 본인인증 후 이용해주세요.");
					exit;
				}
				...
				if($result)
				{
					...
					if($this->input->post("auto_login")){
						...
						$key = md5($_SERVER['HTTP_HOST'] . md5($this->input->post('passwd')));
						setcookie(md5("auto_login"),base64_encode($key),time() + (86400 * 365), '/');
						setcookie(md5("auto_login_uid"),base64_encode($userid),time() + (86400 * 365), '/');

					}
					else{
						setcookie(md5("auto_login"),'',0, '/');
						setcookie(md5("auto_login_uid"),'',0, '/');
					}
					...
					if($this->input->post('go_url')){
						...
						if($edit_date == "0000-00-00" && $register < $date){ //회원 가입 후 6개월 간 비밀번호 변경 내역이 있는지검사
							$view_page = cdir()."/dh_member/change_pw".$go_url;
	...
	public function logout() //유저 로그아웃
	{
		setcookie(md5("auto_login"),'',0, '/');
		setcookie(md5("auto_login_uid"),'',0, '/');
		$array_items = array('USERID' => '', 'PASSWD' => '', 'NAME' => '', 'LEVEL' => '', 'NICKNAME' => '', 'CART' => '');
		...
		if($userChk && $this->input->get("userChkid")){
			...
			$dir = $_SERVER['DOCUMENT_ROOT'].cdir()."/application/views/member/";
				...
				if($this->input->get("agree")==1){
					...
					}else if($this->input->get("agree")==1){
		...
		$data['row'] = $this->common_m->getRow("dh_member","where userid='".$this->db->escape_str($userid)."' and outmode!=1");
			...
			if($this->input->post('idx')){
				...
				if($this->input->post('new_passwd',TRUE)){
					$passwd = $this->member_m->sql_password($this->input->post('new_passwd',TRUE));
				}else{
	...
	public function change_pw($data='') //회원가입 후 6개월간 비밀번호 변경안햇을 시 나오는 페이지
	{
		$data['mem_stat'] = $this->common_m->getRow2("dh_member","where userid='".$this->session->userdata('USERID')."'"); // 유저 정보
				...
				if(!$go_url){ $go_url = "/html/"; }
					...
					$passwd_old = $this->member_m->sql_password($this->input->post('passwd_old'));
	...
	public function leave($data='')
	{
		...
		if($this->input->post('del_idx')){
			...
			if($cnt){
				...
				$member_info = $this->common_m->self_q("select * from dh_member where idx = '".$this->input->post('del_idx')."'","row");
				$deluserid = $member_info->userid;

				$this->common_m->self_q("delete from dh_point where userid = '{$deluserid}'","delete");	//포인트삭제
				$this->common_m->self_q("delete from dh_coupon_use where userid = '{$deluserid}'","delete");	//쿠폰삭제
	...
	{
		if($this->session->userdata('USERID')){
			result(1,'',cdir().'/dh_member/mypage');
			exit;
		}

		$data['find_cnt'] = 0;

		$di = $this->db->escape_str($this->input->post('dupinfo',true));
		$idx = $this->db->escape_str($this->input->post('mem_idx',true));
		$passchg = $this->db->escape_str($this->input->post('passchg',true));
		$post_pass = $this->db->escape_str($this->input->post('passwd',true));
