...
class Dh extends CI_Controller {
	...
	{
		if($_SERVER['HTTP_HOST'] == "m.ecomommeal.co.kr"){	//앱에서 접근한 경우 강제로 보내버림
			alert("https://ecomommeal.co.kr/m");
		}
	...
	public function _remap($method) //모든 페이지에 적용되는 기본 설정.
	{
		...
		if(@$_GET['pcv'] == "Y"){
			$set_session=array('MCHK'=>'Y');
			$this->session->set_userdata($set_session);
			alert("http://".$_SERVER['HTTP_HOST']);
		}
		...
		if($arr || $this->input->get('ajax') == 1 || $method == 'deposit_tmps'){ //개발 페이지 일때
			...
			if($p){
				...
				if($p == 'event04_order_ok'){
					if($_POST){
						...
						if($result){
							alert(cdir()."/dh_order/shop_order_ok/".$result);
						}
					}
				}
					...
					if($_POST){	//쿠폰 검증 로직 쿠폰값 $_POST['coupon_code']
						...
						if($cnt){	//쿠폰이 유효할 경우
							...
							*/
							...
							alert(cdir()."/dh_board/lists/withcons07/?myqna=Y","유효하지 않은 쿠폰번호 입니다. 1:1 문의를 통해 문의해주세요");
						}
					}
				}
				...
				if($p == "srch_view"){
					...
					$find = $this->input->get('sch_value');

					$sql = "select * from dh_goods where display = '1' and cate_no not in ('8') and name like '%{$find}%' order by ranking asc";
					$data['list'] = $this->common_m->self_q($sql,"result");
				...
				if($p == "pro_view"){
					$data['row'] = $this->common_m->self_q("select * from dh_goods where idx = '".$this->input->get('idx')."'","row");
				}
					...
					if($today_week != 0 and $today_week < 5){	//월~목
						...
						while(true){
							if($holis[$sample_deliv_date]){
								//$sample_deliv_date = date("Y-m-d",strtotime('+1 day',strtotime($sample_deliv_date)));
						...
						//샘플 신청자 리스트
						$data['sample_order_user_list'] = $this->common_m->self_q("select a.name, b.goods_name from dh_trade a, dh_trade_goods b where a.trade_code = b.trade_code  and a.sample_is = 'Y' and trade_stat != '9' and b.date_bind = '".$data['sample_deliv_date']."'","result");
						...
						$trade_cnt = $this->common_m->self_q("select a.idx from dh_trade a, dh_trade_deliv_info b where a.trade_code = b.trade_code and b.deliv_date = '".$data['sample_deliv_date']."'  and a.sample_is = 'Y' and trade_stat != '9'","cnt");	//거래내역 카운트
						...
						if($this->session->userdata('USERID')){
							$user_trade_cnt = $this->common_m->self_q("select a.idx from dh_trade a, dh_trade_goods b where a.trade_code = b.trade_code and a.userid = '".$this->session->userdata('USERID')."'  and a.sample_is = 'Y' and trade_stat != '9'","cnt");
						}
				...
				if($p == "free_pay" || $p == "sidedish_free2"){	//골라담기 or 반찬국 골라담기 주문페이지2
					$data['week_name_arr'] = array('일','월','화','수','목','금','토');
				...
				if($p == "free_list" || $p == "sidedish_free1" || $p == "bfood_order_free1"){	//골라담기 or 반찬국 골라담기 주문페이지1
					...
					if($_POST){
						...
						foreach($goods_idx as $k=>$v){

							$tmpcart_cnt = $this->common_m->self_q("select * from dh_freecart_tmp where cart_id = '{$cart_id}' and userid = '{$userid}' and goods_idx = '{$goods_idx[$k]}' and deliv_date = '{$deliv_date}'","cnt");

							if($tmpcart_cnt <= 0){
								$all_ins['cart_id'] = $cart_id;
								$all_ins['userid'] = $userid;
								$all_ins['deliv_date'] = $deliv_date;
								$all_ins['goods_idx'] = $goods_idx[$k];
								$all_ins['price'] = $price[$k];
								$all_ins['origin_price'] = $origin_price[$k];
								$all_ins['goods_name'] = $name[$k];
								$all_ins['cnt'] = 1;
								$all_ins['wdate'] = timenow();

								$result = $this->common_m->insert2("dh_freecart_tmp",$all_ins);
							}
						}
					...
					$data['step_arr'] = array(
						"1-6"=>"<strong>준비기 150g</strong> (5개월 전후)",
						"1-7"=>"<strong>초기 150g</strong> (5~6개월)",
						"1-8"=>"<strong>중기 준비기 150g</strong> (7~8개월)",
						"1-9"=>"<strong>중기 150g</strong> (7~8개월)",
						"1-10"=>"<strong>후기 200g</strong> (9~12개월)",
						"1-11"=>"<strong>완료기 200g</strong> (12개월~)",
						"2-12"=>"<strong>반찬 150g</strong> ",
						"2-13"=>"<strong>국 200g</strong> "
					);

				}

				if($p == "regular_pay" || $p == "sidedish_regular2"){	//추천식단 or 반찬국 추천식단 주문페이지2
				...
				if($p == "regular01" or $p == "bfood_order_regular1"){	//추천식단 or 반찬국 추천식단 주문페이지1
					...
					$data['step_arr'] = array(
						"2"=>"<strong>준비기 150g</strong> (5개월 전후)",
						"4"=>"<strong>초기 150g</strong> (5~6개월)",
						"5"=>"<strong>중기 150g</strong> (7~8개월)",
						"6"=>"<strong>후기2식 200g</strong> (9~12개월)",
						"1"=>"<strong>후기3식 200g</strong> (9~12개월)",
						"7"=>"<strong>완료기 200g</strong> (12개월~)",
						"3"=>"<strong>반찬 150g <small style='font-size:10px;'>(일부품목제외)</small> / 국 200g</strong>"
					);
	...
	public function main($data)
	{
		...
		$data['main_popups'] = $this->common_m->self_q("select *,(select count(*) from dh_bannerlist where parent_code = dh_banner.code and upfile2 <> '' ) as cnt from dh_banner where used = 'Y' order by sorting asc","result");
		$in_cnt = 0;
		...
		//pr($data['main_popups']);
		//$data['main_popups_banner'] = $this->common_m->self_q("select * from dh_bannerlist where parent_code = 'm_popup' order by sort asc","result");
		//메인 배너 & 메인 팝업 배너 종료
		...
		$data['bb_Arr'] = array("<b>","</b>");
		$data['sr_Arr'] = array("<strong>","</strong>");
		$data['main_E_banner'] = $this->common_m->self_q("select * from dh_bannerlist2 where parent_code = 'm_main_event' and addinfo4 = 'used' order by sort desc, idx desc limit 1","row");

		//메인 뉴스
		$data['news'] = $this->common_m->self_q("select * from dh_bbs_data where code = 'withcons01' and notice = 1 order by sort asc, idx desc limit 5","result");
	...
	public function dh_ajax(){	//Ajax _ Json 통합 프로세스
		...
		if($mode == "get_schedule_delivery_item"){	// 추천식단 우측 주문 정보 Ajax.
			...
			//			$vals['delivery_week_type_food_info'] = $delivery_week_type_food_info;
			//			$vals['delivery_start_date'] = $delivery_start_date;
			//			$vals['start_day'] = $start_day;
			//
			//			$ScheduleDeliveryDay = $this->getScheduleDeliveryDay($recom_idx, $start_day, $delivery_start_date, $delivery_week_day_count, $delivery_week_type, $delivery_week_count, $delivery_sun_type);
			//
			//			$content = '';
			//			ob_start();
			...
			if($delivery_week_day_count and $delivery_week_type and $delivery_week_count and $delivery_week_type_key){
	...
	public function main_popup(){
		...
		//$list = $this->common_m->self_q("select * from dh_bannerlist where upfile2 != '' and parent_code = '{$code}' order by sort","result");

		$json = array();
		$json['html'] = $this->main_pop_layer($code);

		echo json_encode($json);
	}

	public function main_pop_layer($code){
		$list = $this->common_m->self_q("select * from dh_bannerlist where upfile2 != '' and parent_code = '{$code}' order by sort","result");

		ob_start();
		include $_SERVER['DOCUMENT_ROOT'].cdir()."/application/views/ajax/ajax.main.pop.banner.php";
		$content = ob_get_contents();
		ob_end_clean();
	...
	public function deposit_tmps($data=''){
		$txt = '';
		...
		$insert['trade_code'] = $this->input->post('P_OID');
		$insert['data'] = $trade_data;
		$insert['trade_day'] = timenow();

		$result = $this->common_m->insert2("dh_trade_tmp",$insert);
		if($result){
			echo "ok";
		}
	}
