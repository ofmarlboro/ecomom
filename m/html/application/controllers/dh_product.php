...
class Dh_Product extends CI_Controller {
	...
	function prod_view($data='')
	{
		...
		if($this->input->get("ajax")==1 && $this->input->get("option_idx")){
			...
			//$cart_deliv_list = $this->common_m->self_q("select * from dh_cart where userid = '".$this->session->userdata('USERID')."' and code = '".$this->session->userdata('CART')."' and trade_ok != '1'","result");
			//$cart_deliv_list = $this->common_m->self_q("select * from dh_cart where userid = '".$this->session->userdata('USERID')."' and trade_ok != '1'","result");
