...
class Dh_board extends CI_Controller {
		...
		if($this->input->post('name') && $this->input->post('pwd'))
		{
			...
			if(isset($_FILES['bbs_file2']['name']) && $_FILES['bbs_file2']['size'] > 0)
			{
				...
				$real_file2 = $data['row']->real_file2;
					...
					if($result)
					{
						...
						alert(cdir()."/".$this->uri->segment(1)."/lists/".$code.$data['query_string'].$data['param'],"수정되었습니다.");

					}
					else
					{
						alert(cdir()."/".$this->uri->segment(1)."/write/".$code.$data['query_string'].$data['param'],"등록에 실패하였습니다.");
					}
			...
			if($this->input->post('pwd_chk') == 1 && $this->input->post('pwd')){ //비밀번호 입력 - 본인글 수정하기
				...
				if($result->cnt > 0){
						...
						alert(cdir()."/".$this->uri->segment(1)."/passwd/".$mode."/".$idx,"비밀번호가 일치하지 않습니다.");
				}
		...
		function passwd($data='')
	{
			...
			$data['row'] = $this->common_m->getRow('dh_bbs_data',"where idx='$idx'"); //게시판 보기
			$code = $data['row']->code;
			$go_url = cdir()."/dh_board//lists/".$code;

			switch($mode)
			{
				case "bbs_view" :
					$table = "dh_bbs_data";
					$data['action'] = cdir()."/dh_board/views/".$idx;
				break;
				case "bbs_edit" :
					$table = "dh_bbs_data";
					$data['action'] = cdir()."/dh_board/edit/".$idx;
				break;
				case "bbs_del" :
					$table = "dh_bbs_data";
				...
				case "bbs_coment_del" :
					$table = "dh_bbs_coment";
					$data['action'] = cdir()."/dh_board/passwd/".$mode."/".$idx."/".$coment_idx;
					$go_url = cdir()."/dh_board/views/".$idx."?coment=1&pwd=".$this->input->post('pwd');
				break;
			}
